...
namespace Leap.Unity {
   ...
   * @since 4.1.2
   */
  public class JumpingMinionFingerDirectionDetector : Detector {
    /**
    ...
    */
    ...
    public void Start () {
      JumpScript.jumpAgain = true;
      minion = GameObject.FindWithTag("PlayerMinionJumping");
      rbMinion = minion.GetComponent<Rigidbody>();
    }

    public void FixedUpdate () {
      if(HandModel != null && HandModel.IsTracked && forwardAngleTo <= OnAngle && backwardAngleTo > OnAngle && JumpScript.jumpAgain) {
        minion.transform.eulerAngles = new Vector3(minion.transform.eulerAngles.x, 180, minion.transform.eulerAngles.z);
        rbMinion.velocity = new Vector3(-2f, 7f, 0f);
        JumpScript.jumpAgain = false;
      }
      if(HandModel != null && HandModel.IsTracked && backwardAngleTo <= OnAngle && forwardAngleTo > OnAngle && JumpScript.jumpAgain) {
        minion.transform.eulerAngles = new Vector3(minion.transform.eulerAngles.x, 180, minion.transform.eulerAngles.z);
        rbMinion.velocity = new Vector3(2f, 7f, 0f);
        JumpScript.jumpAgain = false;
      }
        ...
        if(HandModel != null && HandModel.IsTracked){
          ...
          if(hand != null) {
            ...
            if(HandModel.IsTracked && backwardAngleTo <= OnAngle){
              JumpingMinionPalmDirectionDetector.active = false;
              active = true;
              Activate();
            } else if (!HandModel.IsTracked || backwardAngleTo >= OffAngle) {
              JumpingMinionPalmDirectionDetector.active = true;
              active = false;
              Deactivate();
            }
          }
